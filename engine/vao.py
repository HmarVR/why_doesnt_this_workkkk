...
class VAO:
    def __init__(
        ...
        self.NUL_IMG = self.app.mesh.texture.textures["NUL_IMG"]
    ...
    def pack_uniforms(uniforms_map):
        ...
        for uf_name, uf_data in uniforms_map.items():
            ...
            if uf_data == "float":
                ...
                size = 48  # 3x3 floats, but aligned as mat4 in std140 layout
                align = 16  # aligned as mat4 in std140 layout
            elif uf_data == "mat4":
                size = 64  # 4x4 floats
                align = 16
                ...
                size = 48  # 3x3 ints, but aligned as ivec4 in std140 layout
                align = 16  # aligned as imat4 in std140 layout
            elif uf_data == "imat4":
                size = 64  # 4x4 ints
                align = 16
            elif data_type == 'bool':
...
class VAOs:
    ...
    def get_vao(self, program, fbo, vbo, umap={}, tmap=[]):
        vao = VAO(self.app, program, fbo, vbo, uniforms_map=umap, textures_names=tmap)
        return vao
    
    def get_ins_vao(self, program, fbo, vbo, ibo, umap={}, tmap=[], instance_count=1):
        vao = VAO(self.app, program, fbo, vbo, ibo, umap, tmap, instance_count)
        return vao
        
        

    def add_vao(self, vao_name, program, fbo, vbo, umap={}, tmap=[]):
        self.vaos[vao_name] = VAO(self.app, program, fbo, vbo, uniforms_map=umap, textures_names=tmap)
    
    def add_ins_vao(self, vao_name, program, fbo, vbo, ibo, umap={}, tmap=[], instance_count=1):
        self.vaos[vao_name] = VAO(self.app, program, fbo, vbo, ibo, umap, tmap, instance_count)
